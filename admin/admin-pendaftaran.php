<?php
session_start();
include '../koneksi.php';
?>


<?php
include('../layoutadm/head.php');
?>

<?php
include('../layoutadm/header.php');
?>

<?php
include('../layoutadm/navbar.php');
?>
<!-- DataTables -->
<link rel="stylesheet" href="tempalte/bower_components/datatables.net-bs/css/dataTables.bootstrap.min.css">
<!-- Theme style -->
<div class="box">
    <div class="box-header">
        <?php
        if (isset($_GET['msg'])) {
            echo '<div class="alert alert-success alert-dismissible">' .  $_GET['msg'] . ' </div>';
        }

        ?>
        <h3 class="box-title">Info Pendaftaran</h3>
    </div>
    <!-- /.box-header -->
    <div class="box-body">
        <?php
        // $santri = mysqli_query($conn, "SELECT * FROM nilai_hafalan INNER JOIN santri ON 
        // nilai_hafalan.id_santri = santri.id_santri");
        ?>
        <table id="example" class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th>judul</th>
                    <th>Isi Info</th>
                    <th>Gambar</th>
                    <th>Edit</th>
                </tr>
            </thead>
            <tbody>

                <?php
                $santri = mysqli_query($conn, "SELECT * FROM info_pendaftaran ");
                if (mysqli_num_rows($santri) > 0) {
                    while ($s = mysqli_fetch_array($santri)) {

                ?>


                        <tr>

                            <td><?= $s['judul'] ?></td>
                            <td><?= $s['info'] ?></td>
                            <td><img src="../uploads/pendaftaran/<?= $s['gambar'] ?>" width="150px" alt="">
                            </td>
                            <td> <a href="./admin-edit-pendaftaran.php?id=<?= $s['id'] ?>"><button type="button" class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i></button></td>
                        </tr>

                    <?php }
                } else { ?>

                    <tr>
                        <td>Data tidak ada</td>
                    </tr>
                <?php } ?>
            </tbody>

        </table>
    </div>
    <!-- /.box-body -->
</div>
<!-- /.box -->
<script type="text/javascript">
    window.setTimeout(function() {
        $(".alert").fadeTo(300, 0).slideUp(300, function() {
            $(this).remove();
        });

    }, 3000);
</script>


<?php
include('../layoutadm/footer.php');
?>